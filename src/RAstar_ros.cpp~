...
ofstream excelFile ("RA_result.txt");
...
void RAstarPlannerROS::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
  ...
  if (!initialized_)
  {
	...
	excelFile << "StartID\tStartX\tStartY\tGoalID\tGoalX\tGoalY\tPlannertime(ms)\tpathLength\tnumberOfCells\t" << endl;
...
excelFile << startCell <<"\t"<< start.pose.position.x <<"\t"<< start.pose.position.y <<"\t"<< goalCell <<"\t"<< goal.pose.position.x <<"\t"<< goal.pose.position.y;
...
// convert the path
	...
	excelFile << "\t" <<path_length <<"\t"<< plan.size() <<endl;
      //publish the plan
...
for (uint i=0; i<mapSize; i++)
  ...
  bestPath=findPath(startCell, goalCell,  g_score);
   ...
   excelFile <<"\t"<< (diff(time1,time2).tv_sec)*1e3 + (diff(time1,time2).tv_nsec)*1e-6 ;
